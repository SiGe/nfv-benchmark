...
SWEEP_BUFFERS=(2 8 32 128 512 1024 4096)
...
for pkt in ${PACKET_SIZE[@]}; do
    ...
    for mod1 in ${SWEEP_BUFFERS[@]}; do
        for mod2 in ${SWEEP_BUFFERS[@]}; do
            ...
            make profile-run BENCHMARK=${BENCHMARK} EXTRA="-DREPEAT=200 -DPACKET_SIZE=${pkt} -DMOD_BUFFER_SIZE_2=${mod2} -DMOD_BUFFER_SIZE_1=${mod1} -DMEASUREMENT_BUFFER_SIZE=${mod1}" | grep cycles | rev | cut -d' ' -f1 | sed -e 's/[()]//g' | rev
        done
